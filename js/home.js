...
async function getRandomMeal() {
  ...
  searchedMeal.innerHTML = `
        <p id='ing-tag'>#${data.meals[0].strCategory} &nbsp;&nbsp; #${
    data.meals[0].strArea
  }</p>
        <img src='${
          data.meals[0].strMealThumb
        }' class='searched-meal-img-2' style='height:100px; width:100px;' />
...
async function searchRecipes(query) {
  ...
  if (data.meals != null) {
    data.meals.forEach((el) => {
      ...
      searchedMeal.classList.add("searched-meal-div");
      searchedMeal.innerHTML = `
                <a href='./recipe.view.html?id=${el.idMeal}'>
